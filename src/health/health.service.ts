...
export class HealthService {
  ...
  async getCompleteAddress(geo) {
    ...
    const response = await axios.get(
      "https://api-adresse.data.gouv.fr/search/?q=" + geo
    );
  ...
  async getMeteo(ville) {
    ...
    const response = await axios.get(
      "http://api.openweathermap.org/data/2.5/weather?q=" +
        ville +
        "&appid=9f2e39ff6af958d1980ff533fec45072"
    );
  ...
  async getPollution(lat, lon) {
    ...
    const response = await axios.get(
      "http://api.airvisual.com/v2/nearest_city?lat=" +
        ...
        "&lon=" +
        lon +
        "&key=e9fcaa43-0b53-42d5-8219-d71319e993ce"
    );
    ...
    if (parseInt(qualiAirG["current"]["pollution"]["aqius"]) <= 50) {
      ...
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) <= 100 &&
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) > 50
    ) {
      tmp =
        "Air modérement bon: " +
        qualiAirG["current"]["pollution"][" aqius"] +
        " aqi.";
    } else if (
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) <= 150 &&
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) > 100
    ) {
      tmp =
        "Air insalubre pour les groupes sensibles : " +
        qualiAirG["current"]["pollution"]["aqius"] +
        "aqi.";
    } else if (
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) <= 200 &&
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) > 150
    ) {
      tmp =
        "Air malsain : " + qualiAirG["current"]["pollution"]["aqius"] + " aqi.";
    } else if (
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) <= 300 &&
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) > 200
    ) {
      tmp =
        "Air très malsain : " +
      ...
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) <= 500 &&
      parseInt(qualiAirG["current"]["pollution"]["aqius"]) > 300
    ) {
      tmp =
        "Air dangereux : " +
